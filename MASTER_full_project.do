...
*** B.SCE.11: MONTHIFY SCE BILLING DATA [B.SCE.6 B.SCE.8]
...
***
*** B.GIS.1-B.GIS.13 build the full set of geographic datasets that rely on publicly available spatial data
***
{
...
*** B.GIS.8: CREATE CONCORDANCES BETWEEN CLUs and {COUNTIES and WATER BASINS} [B.GIS.3]
...
*** B.GIS.7: PROCESS TWO CONCORDANCES IN STATA [B.GIS.4 B.GIS.5 B.GIS.6]
...
*** B.GIS.8: PROCESS CLU-PARCEL CONCORDANCE, AND AGGREGATE CLUs UP TO PARCELS [B.GIS.7] 
do "$dirpath_code/build/GIS_Data_Build/BUILD_gis_aggr_clus_parcels.do"

*** B.GIS.9: CONSTRUCT ANNUAL PANEL OF CROPS AT THE FIELD AND FARM LEVELS [B.GIS.7 B.GIS.8]
...
*** B.GIS.11: ASSIGN SURFACE WATER ALLOCATIONS TO CLUS (FOLLOWING NICK HAGERTY) [B.GIS.3]
// "GIS_Data_Build/BUILD_gis_hagerty_clu_assign_revised.R"

*** B.GIS.12: CREATE SUBSET OF SURFACE WATER SHAPEFILES [B.GIS.11]
// "GIS_Data_Build/BUILD_gis_subset_water_district_shapefiles.R" 

*** B.GIS.13: CREATE SUBSET OF SURFACE WATER SHAPEFILES [B.GIS.12]
// "GIS_Data_Build/BUILD_gis_check_clu_wdist_intersections.R"

}


ONCE WE HAVE THE NEW PGE EDRP, WE NEED TO RERUN (MUCH OF) the B.COM section over again

***
*** B.COM.1-B.COM.6 combine PGE and SCE data with other (mostly geographic) datasets
***
{	
*** B.COM.1: ASSIGN LAT/LONS OF PGE SPS, APEP PUMPS, AND SCE SPS TO CLU POLYGONS [B.PGE.11 B.PGE.14 B.SCE.10 B.GIS.3 B.GIS.7]
...
*** B.COM.2: USE CLU ASSIGNMENTS TO LINK PGE SPS, APEP PUMPS, AND SCE SPS TO OTHER POLYGONS [B.GIS.6 B.GIS.7 B.COM.1]
...
*** B.COM.3: CLU/PARCEL DIAGNOSTICS AND GROUP ASSIGNMENTS [B.GIS.8 B.COM.2]
do "$dirpath_code/build/Combined_Data_Build/BUILD_com_assign_clu_parcel_groups.do"

*** B.COM.4: ASSIGN UNITS TO SURFACE WATER POLYGONS, BY CLU AND POINT-WISE [B.GIS.12 B.COM.1 B.COM.3]
...
*** M.PGE.5: CONSTRUCT MONTHLY/HOURLY PANEL DATASETS FOR ELECTRICITY REGRESSIONS [B.PGE.9 B.PGE.12 B.PGE.13 B.GIS.9 B.COM.4 B.COM.5 B.COM.6 M.PGE.1 M.PGE.3 M.PGE.4]
