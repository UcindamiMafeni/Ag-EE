...
** 2. SCE 2020 data pull
{

** Load raw SCE customer EE data
use "$dirpath_data/sce_raw/energy_efficiency_data_20200722.dta", clear
duplicates drop

** Service agreement ID
rename iouserviceaccountid sa_uuid
assert sa_uuid!=""
unique sa_uuid // 198 unique service agreements
...
gen ee_install_date = date(dt,"DMY")
format %td ee_install_date
count if ee_install_date==. // 0 missings out of 8841 observations
la var ee_install_date "Installation date"
drop dt
...
gen ee_paid_date = date(dt,"DMY")
format %td ee_paid_date
count if ee_paid_date==. // 1 missings out of 8841 observations
la var ee_paid_date "Date incentive was paid"
drop dt
...
order sa_uuid ee_install_date claimid sector ee_enduse eega_desc eega_code eega_desc_sub measure_desc ///
	ee_incentive ee_paid_date ee_units quantity_ex_ante savings_kw savings_kwh
drop authorizedsignaturedate iouprojectdescription
sort *
compress
save "$dirpath_data/sce_cleaned/sce_ee_programs_20200722.dta", replace	
}
