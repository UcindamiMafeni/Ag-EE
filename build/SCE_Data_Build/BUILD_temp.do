...
use "$dirpath_data/merged_pge/ag_rates_for_merge.dta", replace
...
local YM_min = ym(2015,12) // fully mergeable data only spans from dec 2015-dec 2019
local YM_max = ym(2019,12)
forvalues YM = `YM_min'/`YM_max' {
	...
	** Keep if in month
	keep if ym(2015,12)==ym(year(bill_start_dt),month(bill_start_dt))
	*format bill_start_dt %td
*egen max_date=max(bill_start_dt)
*egen min_date=min(bill_start_dt)
*	format max_date %td
*	format min_date %td
	...
	** Merge in hourly interval data
	preserve
	clear
	use "$dirpath_data/pge_cleaned/interval_data_hourly_20180322.dta" if inrange(date,`dmin',`dmax')
	...
	** Assign marginal price per kWh for each voltage category
	gen p_kwh = .
