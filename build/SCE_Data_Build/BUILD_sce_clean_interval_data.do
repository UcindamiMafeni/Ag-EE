...
** 1. Clean raw weekly files and collapse to the hourly level (2019 data pull)
{
** Loop over raw (weekly) SCE interval data
clear
cd "$dirpath_data/sce_raw"
local files: dir . files "interval*20190916.dta"
...
** 2. Append weekly datasets and collapse (2019 data pull)
{
...
** 3. Clean raw weekly files and collapse to the hourly level (2020 data pull)
{
** Loop over raw (weekly) SCE interval data
clear
cd "$dirpath_data/sce_raw"
local files: dir . files "interval*20200722.dta"
foreach file of local files {
...
** 4. Append weekly datasets and collapse (2020 data pull)
{
...
sort sa_uuid date hr
compress
save "$dirpath_data/sce_cleaned/interval_data_hourly_20200722.dta", replace

** Collapse to daily level and save
use "$dirpath_data/sce_cleaned/interval_data_hourly_20200722.dta", clear
egen double kwh_daily = sum(kwh), by(sa_uuid date)
drop hr kwh
rename kwh_daily kwh
la var kwh "kWh consumed in daily interval"
...
assert r(unique)==r(N)
compress
save "$dirpath_data/sce_cleaned/interval_data_daily_20200722.dta", replace	

** Collapse to monthly level and save
use "$dirpath_data/sce_cleaned/interval_data_daily_20200722.dta", clear
gen modate = ym(year(date),month(date))
format %tm modate
egen double kwh_monthly = sum(kwh), by(sa_uuid modate)
assert kwh!=.
...
assert r(unique)==r(N)
compress
save "$dirpath_data/sce_cleaned/interval_data_monthly_20200722.dta", replace	
