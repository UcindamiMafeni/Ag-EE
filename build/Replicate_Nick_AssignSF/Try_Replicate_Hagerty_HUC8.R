...
huc8_shp <- huc8_shp %>%
  mutate(huc8_acres=as.numeric(st_area(.)) * m2_to_acre)
...
#collapse allocations to average and trim data
allocations_huc8 <- allocations_huc8 %>%
  ...
  rename(huc8_alloc= vol_deliv_cy_ag) %>%
  distinct(.)
...
inter_clu_huc8 <- readRDS(file.path(path_output, "HUC8_polygon_intersection.rds"))

inter_user_huc8 <- st_intersection(lwgeom::st_make_valid(users_shp), lwgeom::st_make_valid(dauco_shp))
saveRDS(inter_clu_huc8, file.path(path_output, "dauco_polygon_intersection.rds"))
inter_clu_huc8 <- readRDS(file.path(path_output, "dauco_polygon_intersection.rds"))


# Plot of allocations with largest huc8's allocation only

inter <- inter_clu_huc8 %>% 
...
largest_HUC8_user_inter <- union_clus %>%
  group_by(user_id) %>%
  arrange(desc(IntAcres), .by_group = TRUE) %>%
  slice(1)%>%
  ungroup() %>%
  select(user_id,HUC_8,IntAcres,user_acres,huc8_acres)

#HUC8 allocations wtd by fraction of intersected area added
users_largest_alloc <- largest_HUC8_user_inter %>%
  inner_join(.,allocations_polygon) %>%
  inner_join(.,allocations_huc8) %>%
  mutate(frac_area=IntAcres/huc8_acres) %>%
  mutate(user_alloc= frac_area*huc8_alloc + user_alloc) %>%
...
dev.off()
ggsave(file.path(path_out, "huc8_largest_allocations.png"),dpi=300, plot = plot_all)
...
#HUC8 allocations wtd by fraction of intersected area added
users_largest_alloc <- inter %>%
  inner_join(.,allocations_huc8) %>%
  mutate(frac_area=IntAcres/huc8_acres) %>%
  mutate(inter_alloc= frac_area*huc8_alloc) %>%
  group_by(user_id) %>%
  arrange(desc(IntAcres), .by_group = TRUE) %>%
...
dev.off()
ggsave(file.path(path_out, "huc8_all_allocations.png"),dpi=300, plot = plot_all)
