...
** Merge into customer data on SA
joinby sa_uuid using "$dirpath_data/pge_cleaned/pge_cust_detail_20180827.dta", unmatched(both)
assert _merge!=1 // confirm that all SAs in billing data exist in customer data
unique sa_uuid
local uniq = r(unique)
unique sa_uuid if _merge==2 
...
local rN = r(N)
count if temp_neg==1 & flag_nem==1 
di r(N)/`rN' // virtualy all negative bills have a NEM flag, thank god
egen temp_neg_bad_max = max(temp_neg_bad), by(sa_uuid)
br if temp_neg_bad_max!=0 // these are 1-off negative bills, in 3 otherwise normal SAs
drop temp*
