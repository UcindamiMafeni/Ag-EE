...
########################################################################
### 1. Rasterize panels of groundwater readings (all CADWR readings) ###
########################################################################
...
########################################################################
### 2. Rasterize panels of groundwater readings (SAN JOAQUIN VALLEY) ###
########################################################################
...
#Filter on basin == San Joaquin Valley
gwmth <- gwmth %>% filter(basin_id=="5-022")
gwqtr <- gwqtr %>% filter(basin_id=="5-022")
...
for (ym in levels(gwmth$modate)) {
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast1 <- rasterize(temp_idw1, rast, temp_idw1$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwmth_sj_1_",ym),temp1) # store subseted monthly dataset
  assign(paste0("gwmth_sj_rast_1_",ym),temp_rast1) # store monthly raster
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast2 <- rasterize(temp_idw2, rast, temp_idw2$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwmth_sj_2_",ym),temp2) # store subseted monthly dataset
  assign(paste0("gwmth_sj_rast_2_",ym),temp_rast2) # store monthly raster
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast3 <- rasterize(temp_idw3, rast, temp_idw3$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwmth_sj_3_",ym),temp3) # store subseted monthly dataset
  assign(paste0("gwmth_sj_rast_3_",ym),temp_rast3) # store monthly raster
...
for (yq in levels(gwqtr$qtr)) {
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast1 <- rasterize(temp_idw1, rast, temp_idw1$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwqtr_sj_1_",yq),temp1) # store subseted quarterly dataset
  assign(paste0("gwqtr_sj_rast_1_",yq),temp_rast1) # store quarterly raster
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast2 <- rasterize(temp_idw2, rast, temp_idw2$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwqtr_sj_2_",yq),temp2) # store subseted quarterly dataset
  assign(paste0("gwqtr_sj_rast_2_",yq),temp_rast2) # store quarterly raster
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast3 <- rasterize(temp_idw3, rast, temp_idw3$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwqtr_sj_3_",yq),temp3) # store subseted quarterly dataset
  assign(paste0("gwqtr_sj_rast_3_",yq),temp_rast3) # store quarterly raster
...
########################################################################
### 3. Rasterize panels of groundwater readings (SACRAMENTO VALLEY) ####
########################################################################
...
#Filter on basin == Sacramento Valley
gwmth <- gwmth %>% filter(basin_id=="5-021")
gwqtr <- gwqtr %>% filter(basin_id=="5-021")
...
for (ym in levels(gwmth$modate)) {
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast1 <- rasterize(temp_idw1, rast, temp_idw1$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwmth_sac_1_",ym),temp1) # store subseted monthly dataset
  assign(paste0("gwmth_sac_rast_1_",ym),temp_rast1) # store monthly raster
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast2 <- rasterize(temp_idw2, rast, temp_idw2$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwmth_sac_2_",ym),temp2) # store subseted monthly dataset
  assign(paste0("gwmth_sac_rast_2_",ym),temp_rast2) # store monthly raster
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast3 <- rasterize(temp_idw3, rast, temp_idw3$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwmth_sac_3_",ym),temp3) # store subseted monthly dataset
  assign(paste0("gwmth_sac_rast_3_",ym),temp_rast3) # store monthly raster
...
for (yq in levels(gwqtr$qtr)) {
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast1 <- rasterize(temp_idw1, rast, temp_idw1$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwqtr_sac_1_",yq),temp1) # store subseted quarterly dataset
  assign(paste0("gwqtr_sac_rast_1_",yq),temp_rast1) # store quarterly raster
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast2 <- rasterize(temp_idw2, rast, temp_idw2$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwqtr_sac_2_",yq),temp2) # store subseted quarterly dataset
  assign(paste0("gwqtr_sac_rast_2_",yq),temp_rast2) # store quarterly raster
  ...
  res(rast) <- grd_step # set resolution of raster to be same as resolution of IDW grid
  temp_rast3 <- rasterize(temp_idw3, rast, temp_idw3$gw, fun=mean) # RASTERIZE!
  assign(paste0("gwqtr_sac_3_",yq),temp3) # store subseted quarterly dataset
  assign(paste0("gwqtr_sac_rast_3_",yq),temp_rast3) # store quarterly raster
