...
** 2. August 2018 data pull
if 1==1{

** Load raw PGE customer data
use "$dirpath_data/pge_raw/energy_efficiency_data_20180827.dta", clear
duplicates drop

** Service agreement ID
assert sa_uuid!=""
assert length(sa_uuid)==10
unique sa_uuid // 3123 unique service agreements
la var sa_uuid "Service Agreement ID (anonymized, 10-digit)"

** Issue date
gen ee_measure_date = date(chk_issue_date,"MDY")
format %td ee_measure_date
count if ee_measure_date==. // 74 missings out of 5,496 observations
assert chk_issue_date=="" if ee_measure_date==.
la var ee_measure_date "Date that EE application was paid"
drop chk_issue_date
...
assert r(unique)==r(N)
compress
save "$dirpath_data/pge_cleaned/pge_ee_programs_20180827.dta", replace	
