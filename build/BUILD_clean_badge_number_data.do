...
// NOTE: This file is probably irrelevant, as these are non-APEP SP/SAs
// and we really don't need to know anything about their meter IDs

** Load raw PGE customer data
use "$dirpath_data/pge_raw/meter_badge_number_data_20180827.dta", clear
duplicates drop
...
assert length(sp_uuid)==10
duplicates r sp_uuid
unique sp_uuid // 68654 unique service points
unique sp_uuid pge_badge_nbr // 69277 unique service point-meters (ALMOST unique)
la var sp_uuid "Service Point ID (anonymized, 10-digit)"

** PGE meter badge number
assert pge_badge_nbr!="" 
count if length(pge_badge_nbr)==10 // 87% have 10 digits
gen len = length(pge_badge_nbr)
tab len
drop len
unique pge_badge_nbr // 15091 unique badge numbers 
...
** Save
compress
save "$dirpath_data/pge_cleaned/meter_badge_number_data_20180827.dta", replace	
