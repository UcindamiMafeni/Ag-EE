...
use "$dirpath_data/pge_raw/bill_data_20180322.dta", clear
...
** 2. July 2018 data pull (which for this data file is actually the August 2 2018 pull)
if 1==0{
...
assert sa_uuid!=""
assert length(sa_uuid)==10
unique sa_uuid // 25072 unique service agreements
unique sa_uuid bseg_start_dt // 1035117 unique customer-service point-service aggrements
duplicates r sa_uuid bseg_start_dt
duplicates t sa_uuid bseg_start_dt, gen(dup)
*br if dup>0 // a BUNCH of dups (almost 1% of account-bills), which will be tricky to parse!
drop dup
...
** Start and end dates
assert bseg_start_dt!="" & bseg_end_dt!=""
gen bill_start_dt = date(bseg_start_dt,"MDY")
gen bill_end_dt = date(bseg_end_dt,"MDY")
format %td bill_start_dt bill_end_dt
assert bill_start_dt!=. & bill_end_dt!=. // no missings
assert bill_start_dt<=bill_end_dt // end date never prior to start date
gen bill_length = bill_end_dt-bill_start_dt+1
tab bill_length // 0.3% of observations have bill period > 34 days! (97% b/tw 28-34 days)
drop bseg_start_dt bseg_end_dt
order sa_uuid bill_start_dt bill_end_dt bill_length
la var bill_start_dt "Bill period start date"
la var bill_end_dt "Bill period end date"
...
** Usage and demand
destring total_electric_usage max_demand peak_demand partial_peak_demand, replace
count if total_electric_usage==. // 405 missings out of 1.04M
count if max_demand==. // 179K missings out of 1.04M
count if peak_demand==. // 998K missings out of 1.04M
count if partial_peak_demand==. // 971K missings out of 1.04M
...
sort sa_uuid bill_start_dt bill_end_dt rt_sched_cd
br if dup>0
unique sa_uuid bill_start_dt bill_end_dt rt_sched_cd if dup>0 // 6997 dups
unique sa_uuid bill_start_dt bill_end_dt rt_sched_cd if dup>0 & total_bill_kwh==0
	// 2/3 of dups have a 0 kWh as at least 1 of the dups
egen temp = max(total_bill_kwh==0), by(sa_uuid bill_start_dt bill_end_dt rt_sched_cd)
foreach v of varlist total_bill_kwh total_bill_amount {
...
sort sa_uuid bill_start_dt bill_end_dt rt_sched_cd
br if dup>0
unique sa_uuid bill_start_dt bill_end_dt rt_sched_cd if dup>0 // 2112 dups
foreach v of varlist total_bill_kwh total_bill_amount {
	egen double temp = sum(`v'), by(sa_uuid bill_start_dt bill_end_dt rt_sched_cd)
	replace `v' = temp if dup>0
	drop temp
...
** Duplicate bills: same account, same start/end dates, different kwh, different tariffs
duplicates t sa_uuid bill_start_dt bill_end, gen(dup)
tab dup //  <0.03% of total observations 
sort sa_uuid bill_start_dt bill_end_dt total_bill_kwh
gen temp1 = sa_uuid==sa_uuid[_n+1] & dup==0 & dup[_n+1]>0 // to grab tariff of following bill
gen temp2 = sa_uuid==sa_uuid[_n-1] & dup==0 & dup[_n-1]>0 // to grab tariff of preceding bill
br if dup>0 | temp1==1 | temp2==1
...
replace temp_to_sum = 1 if temp_keep==1 & dup2>0 & temp_BALD==1 & temp_BBLD==1 & ///
	...
	((rt_sched1==rt_sched_cd[_n+1])+(rt_sched2==rt_sched_cd[_n+1]))==1 & ///
	rt_sched_cd[_n-1]!=rt_sched_cd[_n+1] & sa_uuid==sa_uuid[_n-1] & sa_uuid==sa_uuid[_n+1]
tab temp_to_sum if temp_keep==1 // only 5 out of 64 dups, so this is not a good strategy
...
gen temp_date_overlap = (bill_start_dt<bill_end_dt[_n-1] & sa_uuid==sa_uuid[_n-1]) | ///
	(bill_end_dt>bill_start_dt[_n+1] & sa_uuid==sa_uuid[_n+1])
tab temp_date_overlap // 0.06% of bills
br sa_uuid-bill_length total_bill_kwh temp_date_overlap flag_maybe_adjust_end_date ///
	if temp_date_overlap
...
gen temp_date_overlap2 = (bill_start_dt<bill_end_dt[_n-1] & sa_uuid==sa_uuid[_n-1]) | ///
	(bill_end_dt>bill_start_dt[_n+1] & sa_uuid==sa_uuid[_n+1])
tab temp_date_overlap2 // 0.03% of bills
br sa_uuid-bill_length total_bill_kwh temp_date_overlap2 flag_maybe_adjust_end_date ///
	if temp_date_overlap2
...
gen temp_date_overlap3 = (bill_start_dt<bill_end_dt[_n-1] & sa_uuid==sa_uuid[_n-1]) | ///
	(bill_end_dt>bill_start_dt[_n+1] & sa_uuid==sa_uuid[_n+1])
tab temp_date_overlap3 // 0.03% of bills
br sa_uuid-bill_length total_bill_kwh temp_date_overlap3 flag_maybe_adjust_end_date ///
	if temp_date_overlap3
...
gen temp_date_overlap4 = (bill_start_dt<bill_end_dt[_n-1] & sa_uuid==sa_uuid[_n-1]) | ///
	(bill_end_dt>bill_start_dt[_n+1] & sa_uuid==sa_uuid[_n+1])
tab temp_date_overlap4 // 0.03% of bills
br sa_uuid-bill_length total_bill_kwh temp_date_overlap4 flag_maybe_adjust_end_date ///
	if temp_date_overlap4 | temp_date_overlap4[_n-1] | temp_date_overlap4[_n+1]
...
egen ender = max(flag_last_bill), by(sa_uuid)
unique sa_uuid
unique sa_uuid if starter
unique sa_uuid if ender
unique sa_uuid if starter & ender
gen temp_start_month = modate_st if temp_first_start==bill_start_dt
gen temp_end_month = modate_end if temp_last_end==bill_end_dt
egen start_month = mean(temp_start_month), by(sa_uuid)
egen end_month = mean(temp_end_month), by(sa_uuid)
...
local N = r(N)
count if flag_long_bill==0 & total_bill_kwh!=. & total_bill_kwh!=0 & total_bill_kwh>0
di 1 - r(N)/`N' // 0.3% of positive bills are longer than 34 days
count if flag_long_bill==0 & flag_short_bill==0 & total_bill_kwh!=. & total_bill_kwh!=0 & total_bill_kwh>0
di 1 - r(N)/`N' // 3.1% of positive bills are shorter than 28 days
...
assert sa_uuid!=""
assert length(sa_uuid)==10
unique sa_uuid // 109985 unique service agreements
unique sa_uuid bseg_start_dt // 4307716 unique customer-service point-service aggrements
duplicates r sa_uuid bseg_start_dt
duplicates t sa_uuid bseg_start_dt, gen(dup)
*br if dup>0 // a BUNCH of dups (almost 1% of account-bills), which will be tricky to parse!
drop dup
...
** Rate schedule
count if rt_sched_cd=="" // only 4 observations missing
sort sa_uuid bill_start_dt
egen temp1 = mode(rt_sched_cd), by(sa_uuid)
egen temp2 = max(rt_sched_cd==""), by(sa_uuid)
assert (rt_sched_cd==temp1 | rt_sched_cd=="") if temp2==1 // missings are unambiguous
replace rt_sched_cd = temp1 if rt_sched_cd=="" // populate missing
drop temp1 temp2
la var rt_sched_cd "Rate schedule at end of billing cycle"
...
** Usage and demand
destring total_electric_usage max_demand peak_demand partial_peak_demand, replace
count if total_electric_usage==. // 662 missings out of 4.34M
count if max_demand==. // 2.71M missings out of 4.34M
count if peak_demand==. // 4.27M missings out of 4.34M
count if partial_peak_demand==. // 4.26M missings out of 4.34M

** Bill amount
destring total_bill_amount, replace
assert total_bill_amount!=. // never missing!
gen perkwh = total_bill_amount/total_electric_usage
sum perkwh, detail // p5 = 10 cents/kWh, p95 = 6.7 $/kWh (!!)
drop perkwh
...
sort sa_uuid bill_start_dt bill_end_dt rt_sched_cd
br if dup>0
unique sa_uuid bill_start_dt bill_end_dt rt_sched_cd if dup>0 // 6809 dups
foreach v of varlist total_bill_kwh total_bill_amount {
	egen double temp = sum(`v'), by(sa_uuid bill_start_dt bill_end_dt rt_sched_cd)
	replace `v' = temp if dup>0
	drop temp
...
** Duplicate bills: same account, same start/end dates, same kwh, different tariffs
duplicates t sa_uuid bill_start_dt bill_end total_bill_kwh, gen(dup)
tab dup //  0.22% of total observations 
sort sa_uuid bill_start_dt bill_end_dt total_bill_kwh
gen temp1 = sa_uuid==sa_uuid[_n+1] & dup==0 & dup[_n+1]>0 // to grab tariff of following bill
gen temp2 = sa_uuid==sa_uuid[_n-1] & dup==0 & dup[_n-1]>0 // to grab tariff of preceding bill
br if dup>0 | temp1==1 | temp2==1
...
** Duplicate bills: same account, same start/end dates, different kwh, different tariffs
duplicates t sa_uuid bill_start_dt bill_end, gen(dup)
tab dup //  <0.01% of total observations 
sort sa_uuid bill_start_dt bill_end_dt total_bill_kwh
gen temp1 = sa_uuid==sa_uuid[_n+1] & dup==0 & dup[_n+1]>0 // to grab tariff of following bill
gen temp2 = sa_uuid==sa_uuid[_n-1] & dup==0 & dup[_n-1]>0 // to grab tariff of preceding bill
br if dup>0 | temp1==1 | temp2==1
...
restore
	// only 1.6% of accounts are neither openers nor closers!
	// 35.4% of accounts are openers only
	// 27.8% of accounts are closers only
	// for 38.6% of accoutns, we observe the opening and closing!
...
di 1 - r(N)/`N' // 0.5% of positive bills are longer than 34 days
count if flag_long_bill==0 & flag_short_bill==0 & total_bill_kwh!=. & total_bill_kwh!=0 & total_bill_kwh>0
di 1 - r(N)/`N' // 2.6% of positive bills are shorter than 28 days
...
assert sa_uuid!=""
assert length(sa_uuid)==10
unique sa_uuid // 104721 unique service agreements
unique sa_uuid start_dt // 4307716 unique customer-service point-service aggrements
duplicates r sa_uuid start_dt
duplicates t sa_uuid start_dt, gen(dup)
*br if dup>0 // a BUNCH of dups (almost 1% of account-bills), which will be tricky to parse!
drop dup
la var sa_uuid "Service Agreement ID (anonymized, 10-digit)"
...
gen bill_start_dt = date(start_dt,"DMY")
gen bill_end_dt = date(end_dt,"DMY")
format %td bill_start_dt bill_end_dt
assert bill_start_dt!=. & bill_end_dt!=. // no missings
assert bill_start_dt<=bill_end_dt // end date never prior to start date
gen bill_length = bill_end_dt-bill_start_dt+1
tab bill_length // 0.09% of observations have bill period > 34 days! (97% b/tw 28-34 days)
...
** Rate schedule
count if rt_sched_cd=="" // 0 missings
assert rt_sched_cd!=""
tab rt_sched_cd
sort sa_uuid bill_start_dt
la var rt_sched_cd "Rate schedule at end of billing cycle"

destring revn kwh*, replace

save "$dirpath_data/pge_cleaned/sept2020_bills_temp.dta"


// GOING OFF THE RAILS HERE TO DO DATA CHECKS

use "$dirpath_data/pge_cleaned/billing_data_20180322.dta", clear
keep if bill_start_dt>=mdy(6,1,2017)
gen pull = "20180322"
append using  "$dirpath_data/pge_cleaned/billing_data_20180719.dta"
keep if bill_start_dt>=mdy(6,1,2017)
replace pull = "20180719" if pull==""
append using  "$dirpath_data/pge_cleaned/billing_data_20180827.dta"
keep if bill_start_dt>=mdy(6,1,2017)
replace pull = "20180827" if pull==""
duplicates drop sa_uuid bill_start_dt bill_end_dt, force
merge 1:m sa_uuid bill_start_dt bill_end_dt using "$dirpath_data/pge_cleaned/sept2020_bills_temp.dta"
drop if bill_start_dt>mdy(10,1,2017)
tab _merge
correlate total_bill_kwh kwh if _merge==3
gen temp = total_bill_kwh - kwh
sum temp, detail
correlate total_bill_amount rev if _merge==3
gen temp2 = total_bill_amount - rev
sum temp2, detail

use "$dirpath_data/pge_cleaned/pge_cust_detail_202009", clear
keep sa_uuid
duplicates drop 
merge 1:m sa_uuid using "$dirpath_data/pge_cleaned/sept2020_bills_temp.dta"
...
keep pge_badge_nbr 
duplicates drop
merge 1:m pge_badge_nbr using "$dirpath_data/pge_raw\meter_badge_number_data_202009.dta" 
...
egen temp = group(pge_badge_nbr)
egen temp2 = group(pge_badge_nbr mtr_id)
drop temp*
rename pge_badge_nbr BadgeNbr
joinby BadgeNbr using "$dirpath_data/pge_raw/pump_test_data_202009.dta", unmatched(both)
tab _merge
unique BadgeNbr if _merge==3
unique BadgeNbr if _merge==2
rename _merge _merge_history 
keep if _merge_history==2 | _merge_history==3
rename BadgeNbr pge_badge_nbr
joinby pge_badge_nbr using "$dirpath_data/pge_cleaned/pge_cust_detail_202009", unmatched(both)



asdfsdfsafsadfsdfsdfsadfsdfsdafsdfs
