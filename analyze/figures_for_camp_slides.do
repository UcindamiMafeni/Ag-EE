...
keep sp_uuid sa_uuid
duplicates drop
joinby sa_uuid using "$dirpath_data/pge_cleaned/billing_data.dta" , unmatched(master)
tab _merge
drop if _merge==1 
drop _merge
...
twoway hist total_bill_mwh_top50 ///
	...
	ylab(0(300)1200,nogrid angle(0) labsize(vlarge)) ytitle("Number of service points", size(vlarge)) ///
	graphr(color(white) lc(white)) ///
