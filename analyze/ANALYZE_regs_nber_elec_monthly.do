...
******************************************************************
** Script to run monthly electricity regressions for NBER paper **
******************************************************************
...
foreach c of numlist 1/54 {
	...
	if `c'==13 {
		...
		local FEs = "sp_group year"
		local RHS = "log_p_mean"
		local if_sample = "if flag_nem==0 & flag_geocode_badmiss==0 & flag_irregular_bill==0 & flag_weird_cust==0 & merge_sp_water_panel==3 & flag_partial_year==0 & elec_binary_frac>0.9"
	}
	if `c'==14 {
		local DEPVAR = "ihs_kwh"
		local FEs = "sp_group#month modate"
...
dropmiss, obs force
compress
save "$dirpath_data/results/regs_nber_elec_monthly.dta", replace
